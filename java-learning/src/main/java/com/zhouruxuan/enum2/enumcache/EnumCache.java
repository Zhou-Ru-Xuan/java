...
public class EnumCache {
    ...
    private static <E extends Enum> E find(Class<E> clazz, Object obj, Map<Class<? extends Enum>, Map<Object, Enum>> cache, E defaultEnum) {
        ...
        if (map == null) {
            ...
            if (map == null) {
                throw new RuntimeException(generateErrorMsg(clazz, cache));
            }
        }
        return obj == null ? defaultEnum : (E) map.getOrDefault(obj, defaultEnum);
    }
     ...
     * @return 错误信息
     */
    private static String generateErrorMsg(Class<? extends Enum> clazz, Map<Class<? extends Enum>, Map<Object, Enum>> cache) {
        String msgTemplate = "枚举%s还没有注册到枚举缓存中，请在%s.static代码块中加入如下代码 : EnumCache.%s(%s.class, %s.values(), %s::getXxx);";
        String registerMethod = cache == CACHE_BY_NAME ? "registerByName" : "registerByValue";
        return String.format(msgTemplate, clazz.getSimpleName(), clazz.getSimpleName(), registerMethod, clazz.getSimpleName(), clazz.getSimpleName(), clazz.getSimpleName());
    }
